		...
		<view class="content" style="padding-top: 400rpx;">
			...
			<view class="list" v-for="item in ListData">
				...
				<view class="list_right">
					<button class="infoBtn" type="default" @tap='lookBtn(item)'>查看详情</button>
					<text>注册时间:</text>
...
<script>
	...
	export default {
		...
		bindPickerChange: function(key, e, select) {
			...
			selectfuc() {
				...
				this.tabList = dic.reduce((t, v) => {
					k.indexOf(v.dict.dictName) >= 0 && t.push({
		...
		onPullDownRefresh() {
			this.getList(undefined, undefined, true);
